...
for nodeIndex  = 1: length(nodeID_DCCCD)
    nodeID = string(nodeID_DCCCD(nodeIndex));
    for n  = 1:length(mintsTargets) 
        ...
        eval(strcat('mintsNode_',nodeID,'(n,1).modelVersion      =  mintsMdlVersions_DCCCD{n};'))
        if n<= 6
            eval(strcat('mintsNode_',nodeID,'(n,1).inputs           =  mintsInputsPalas_DCCCD;'))
            eval(strcat('mintsNode_',nodeID,'(n,1).inputLabels      =  mintsInputLabelsPalas_DCCCD;'))
        else
            eval(strcat('mintsNode_',nodeID,'(n,1).inputs           =  mintsInputsAirMar_DCCCD;'))
            eval(strcat('mintsNode_',nodeID,'(n,1).inputLabels      =  mintsInputLabelsAirMar_DCCCD;'))
        end 
    end
end
